...
(async()=>{
  ...
  await fs.writeFile('ccu002_02-thrombophilia-potential-cases.csv', 'patient-id,dob,codes,last-encounter\n');
  for(let patient in patients) {
    try {
      const row = patient+','+ages[patient]+',\"'+Array.from(patients[patient]).join(',')+'\",'+lastEncounters[patient].slice(0,-1)+'\n';
      console.log("row:" + row);
      await fs.appendFile('ccu002_02-thrombophilia-potential-cases.csv', row);
    } catch(error) {
      console.log(error);
    }
  }
